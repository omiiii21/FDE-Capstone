...
The reverse would also move me. If the model path is clearly better on verified resolution and does not push grounding-guardrail activations above the 93 seen on the extractive run, then the extractive path is a genuine quality regression during an outage rather than a graceful one, and CloudServe should be told to budget for a paid tier with an uptime commitment instead of relying on it.
