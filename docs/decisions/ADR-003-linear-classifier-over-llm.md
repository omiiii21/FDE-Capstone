...
Zero-shot classification through the provider was the first thing I tried, because it needs no training data and it handles a new class the day someone invents it. It scored lower than the linear model, which I half expected. I should be exact about the standing of that sentence, because it is the weakest evidence in this record: it was an informal comparison over a few dozen tickets during week one, run against a provider I was still wiring up, and I did not keep the artefact. There is no file in `evaluation/results/` behind it and it should not be quoted as a measurement. It is the reason I stopped pursuing the approach, not proof that the approach is worse. The part that ruled it out is that the confidence it reported did not move with correctness: wrong answers came back as assured as right ones, so there was nothing to threshold. It is also not deterministic in a way I can show a compliance reviewer, and A5 requires the same ticket to route the same way twice.
...
Calibration is where the work went, and where I was wrong twice. Expected calibration error uncalibrated is 0.161, against 0.028 for what ships. Both are measured on the 89 tickets of the held-out fold, which are the tickets the calibrator is fitted on, so the 0.028 is in-sample for the calibrator even though it is out-of-sample for the classifier. There is no clean held-out calibration figure and this record should not be read as offering one. Temperature scaling was what I reached for first, and it gets ECE to 0.062, which looks like a success until you see what it does: the only temperature that calibrates a 22-way distribution this peaked is one that sharpens it, around 0.3 to 0.4, and at that setting more than 95% of predictions read 1.000. The metric improves and the routing threshold is left with nothing to separate. The shipped calibrator gets ECE to 0.028 and keeps the confidences spread out, which is the property the router actually needs.
