...
Then `check_injection` runs on the ticket text, before routing rather than after generation, because a ticket that tries to reprogram the system is worth recording whether or not it would have been answered. Routing evaluates in order: R-00 kill switch, R-01 policy class, R-02 no grounding, R-03 high-cost class below the 0.85 bar, R-04 below the 0.62 threshold, R-05 answer. Two further rules live in `src/pipeline.py` rather than in `src/route.py`, because they depend on things the router is not given: R-01b downgrades an auto-respond to an escalation when the injection check has flagged the incoming ticket, and R-06 does the same when the generator declines to write an answer it can stand behind. `src/route.py`'s own docstring lists six rules and is right about its own file; this document describes the path a ticket actually takes, which has eight. Deployment failure is not a policy class and not high-cost, so a confident prediction with grounding lands on R-05. The draft goes through PR-01 if a provider is configured and through the extractive path if not, and then every guardrail runs on whatever is about to leave. If one blocks, the action becomes `blocked`, which means a person gets it with the block reason on the front of the handover note. Never a redacted version to the customer.
